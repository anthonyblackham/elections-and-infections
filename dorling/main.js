...
var radius = d3.scaleSqrt().range([10,30]);
...
d3.queue()
  ...
  .defer(d3.csv, 'data/vaccines.csv', function(d)  {
...
function main(error, geojson) {
  ...
  var partitions = [];
  for (var i = 1; i < 4; i++){
    partitions.push((extent[1] / 3)*i);
  }

  color.domain([0.3, extent[1]])
  .range(partitions.map(function(d, i) {
    return d3.interpolateRdYlBu(i / (3 - 1));
  }));
